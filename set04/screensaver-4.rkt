...
;; RETURNS: a Scene that portrays the given
;;          scene with a dot at the center of
;;          rectangles after every tick on
;;          pen-down on given rectangles.
;; STRATEGY: Use template for ListOfRectangle on lor
(define (create-dots lor canvas)
  (cond
    [(empty? lor) canvas]
    [else (add-pen-down-circle (rect-lod (first lor)) 
                               (create-dots (rest lor)
                                            canvas))]))

;; add-pen-down-circle : ListOfDots Scene -> Scene
;; GIVEN: a list of dots and a scene
;; RETURNS: a Scene that portrays the given
;;          scene with a list of dots at the 
;;          center of rectangle after every 
;;          tick on pen-down on given rectangle.
;; STRATEGY: Use template for ListOfDots on lod
(define (add-pen-down-circle lod canvas)
  (cond
    [(empty? lod) canvas]
    [else (place-dot (first lod)
                     (add-pen-down-circle (rest lod)
                                          canvas))]))

;; place-dot : Dot Scene -> Scene
;; GIVEN: a dot and a scene
;; RETURNS: a Scene that portrays the given
;;          scene with a dot at the center of
;;          rectangle after every tick on
;;          pen-down on given rectangle.
;; STRATEGY: Use template for Dot on dot
(define (place-dot dot canvas)
...
;; GIVEN: the state of a rectangle
;; RETURNS: the state of the rectangle on the next
;;          tick if the pen is down and the rectangle
...
;;          the canvas with reversed velocitites.
;; STRATEGY : Use template for Rect on rect

...
;; screensaver-5.rkt
;; Implementing a screensaver with retractable pen using HOF wherever
;; possible and appropriate.

;; A screensaver that displays rectangles that move around a canvas 
...
;; Initially the simulation is paused and there are no rectangle.
;; Hitting the "n" key adds a new rectangle, at the center of the
;; canvas, at rest (velocity is 0).

;; When a rectangle is selected, the arrow keys increase the velocity
;; of the rectangle in the specified direction (up, down, left, or right).
;; Each push of the arrow key increases the velocity by 2 pixels/tick.
;; When the rectangle is deselected, the rectangle resumes its motion with
;; the new velocity.

;; When a rectangle is selected, the "d" key drops a pen down. When the
;; pen is down, the rectangle records on the screen a dot marking its
;; center at each tick. The dot is displayed as a solid black circle of
;; radius 1.

;; When a rectangle is selected, the "u" key lifts the pen up. When the
;; pen is up, the rectangle does not leave a track on the screen. No
;; marks are made during a drag.
...
;; pen down circle
(define PEN-DOWN-CIRCLE (circle 1 "solid" "black"))
...
;; increase in speed on arrow keyvent in pixels per tick
...
;; A Struct can be Dot or Rectangle
;; A ListOfStruct (LOS) is either
;; -- empty
;; -- (cons Struct LOS)
;; A ListOfStruct can be the ListOfDots or ListOfRectangle
...
(define-struct rect (x y vx vy selected? selected-x selected-y pen-down? lod))
...
;; selected? tests whether the current rectangle is selected or not.
...
;; pen-down? tests whether the pen in the given rectangle is down or not
;; lod represents a ListOfDots of a rectangle when the pen is down.
...
;; Interpretation:
;; A Dot is a (make-dot x y)
;; x is the x coordinate of dot
;; y is the y coorinate of dot
...
(define dot-list
  (list (make-dot CANVAS-CENTER-X CANVAS-CENTER-Y)))

(define pen-down-rect-1-after-tick
  (make-rect CANVAS-CENTER-X
             CANVAS-CENTER-Y
             INITIAL-VELOCITY-X
             INITIAL-VELOCITY-Y
...
(define sample-unpaused-world-after-tick
  (make-world sample-list-after-tick false))
...
(begin-for-test
  (check-equal?
   (initial-world 1)
   initial-world-state
   "The initial paused world state should be returned having
    no rectangles")
  (check-equal?
   (initial-world "anb")
   initial-world-state
...
(define (place-selection-circle lor canvas)
  (foldr
   ...
   (lambda (r canvas) (if (place-selection-circle-condition? r)
                          (add-circle-to-scene r canvas)
                          canvas))
   canvas
   lor))
...
(define (add-circle-to-scene rect canvas)
  (add-image-to-scene rect canvas CIRCLE-IMAGE rect-selected-x rect-selected-y))
#|
(define (add-circle-to-scene rect canvas)
...
(define (place-rects lor canvas)
  (place-struct lor canvas place-rect))
#|
(define (place-rects lor canvas)
  (cond
    [(empty? lor) canvas]
...
;; create-dots : ListOfRectangle Scene -> Scene
;; GIVEN: a list of rectangle and a scene
;; RETURNS: a Scene that portrays the given
;;          scene with list of dots painted
;;          on it of given rectangles.
;; STRATEGY: Use template for ListOfRectangle on lor
(define (create-dots lor canvas)
  (cond
    [(empty? lor) canvas]
    [else (place-dot (rect-lod (first lor)) 
                     (create-dots (rest lor)
                                  canvas))]))

;; place-dot : ListOfDots Scene -> Scene
;; GIVEN: a list of dots and a scene
;; RETURNS: a Scene that portrays the given
;;          scene with a given list of dots
;;          painted on it.
;; STRATEGY: Call a more generic function
(define (place-dot lod canvas)
  (place-struct lod canvas add-dot-to-scene))
#|
(define (place-dot lod canvas)
  (cond
    [(empty? lod) canvas]
    [else (add-dot-to-scene (first lod)
                     (place-dot (rest lod)
                                          canvas))]))|#

;; add-dot-to-scene : Dot Scene -> Scene
;; GIVEN: a dot and a scene
;; RETURNS: a Scene that portrays the given
;;          scene with a dot painted on it.
;; STRATEGY: Call a more generic function
(define (add-dot-to-scene dot canvas)
  (add-image-to-scene dot canvas PEN-DOWN-CIRCLE dot-x dot-y))
#|
(define (add-dot-to-scene dot canvas)
  (place-image PEN-DOWN-CIRCLE
               (dot-x dot)
               (dot-y dot)
               canvas))
|#

;; place-struct : ListOfStruct Scene (Struct Scene -> Scene) -> Scene
;; GIVEN: a list of structs, a scene and a function
;; RETURNS: a Scene that portrays the given
;;          scene with a given list of structs
;;          painted on it.
;; STRATEGY: Use template for ListOfStruct on los
(define (place-struct los canvas func)
  (cond
    [(empty? los) canvas]
    [else (func (first los)
                (place-struct (rest los)
                              canvas func))]))

;; add-image-to-scene : Struct Scene IMAGE Integer Integer -> Scene
;; GIVEN: a struct, a scene, an image and center
;;        coordinates of given struct
;; RETURNS: a Scene that portrays the given
;;          scene with the given image painted on it.
;; STRATEGY: Use template for Struct on struct
(define (add-image-to-scene struct canvas image struct-x struct-y)
  (place-image image
               (struct-x struct)
               (struct-y struct)
               canvas))
...
(define initial-scene
  (place-selection-circle (world-rects initial-world-state)
                          (create-dots (world-rects initial-world-state)
                                       (place-rects
                                        (world-rects initial-world-state)
                                        EMPTY-CANVAS))))
(define sample-scene
  (place-selection-circle (world-rects sample-world)
                          (create-dots (world-rects sample-world)
...
(define selected-scene
  (place-selection-circle (world-rects selected-world-state)
                          (create-dots (world-rects selected-world-state)
                                       (place-rects
                                        (world-rects selected-world-state)
                                        EMPTY-CANVAS))))

(define pen-down-world-state
  (make-world (list pen-down-rect-1-after-tick)  false))

(define pen-down-world-scene
  (place-selection-circle (world-rects pen-down-world-state)
                          (create-dots (world-rects pen-down-world-state)
                                       (place-rects
                                        (world-rects pen-down-world-state)
                                        EMPTY-CANVAS))))
...
(begin-for-test
  ...
  (check-equal?
   (world-to-scene pen-down-world-state)
   pen-down-world-scene
   "Image of pen-down-world-state should be created on the given canvas"))
...
(define (rectangle-after-tick lor)
  (map
   ;; Rectangle -> Rectangle
   (lambda (r) (if (rect-selected? r)
                   r
                   (unselected-rectangle-after-tick r)))
   lor))
...
#|(define (rectangle-after-tick lor)
  (cond
    ...
    [else (if (rect-selected? (first lor))
              ...
              (cons (unselected-rectangle-after-tick (first lor))
                    (rectangle-after-tick (rest lor))))]))
|#

;; unselected-rectangle-after-tick : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of a given rectangle after a 
;;          tick if it were in an unpaused world and
;;          is unselected.
;; STRATEGY: combine simpler functions
(define (unselected-rectangle-after-tick r)
...
(define (move-rectangle-normal rect)
  (if (rect-pen-down? rect)
      ...
      (rect-after-pen-up rect)))|#

;; move-rectangle :
;; Rectangle (Rectangle -> Rectangle) (Rectangle -> Rectangle) -> Rectangle
;; GIVEN: the state of a rectangle and two functions
;; RETURNS: the state of the rectangle on the next
;;          tick depending upon whether the
;;          pen is down or not
;; STRATEGY : Use template for Rect on r
(define (move-rectangle rect func1 func2)
  (if (rect-pen-down? rect)
      (func1 rect)
      (func2 rect)))
...
;; GIVEN: the state of a rectangle
;; RETURNS: the given rectangle after adding a
;;          dot at the center of the given 
;;          rectangle in its list of dots.
;; STRATEGY : Use template for Rect on r
...
(begin-for-test
  (check-equal?
   (move-rectangle-normal pen-down-rect-1)
   (rect-after-pen-down pen-down-rect-1)
   "Rectagle should have a dot added to its
    list of dots if the pen is down"))

;; rect-after-pen-up : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of the rectangle on the next
;;          tick if the pen is up and the rectangle
;;          would not hit or go past the canvas on
;;          the next tick.
;; STRATEGY : Use template for Rect on r
(define (rect-after-pen-up rect)
  (make-rect (+ (rect-x rect) (rect-vx rect))
             (+ (rect-y rect) (rect-vy rect))
             (rect-vx rect)
             (rect-vy rect)
...
;; add-dot : Rectangle -> ListOfDots
;; GIVEN: a rectangle
;; RETURNS: the list of dots of given rectangle
;;          after adding a dot
;; STRATEGY: Use template for Rectangle on rect
(define (add-dot rect)
  (cons (make-dot (rect-x rect) (rect-y rect))
        (rect-lod rect)))
...
(begin-for-test
  (check-equal?
   (bounce-rect-from-top (make-rect -1 -1 -1 -1 false 0 0 false empty))
   (rect-after-perfect-bounce-from-left-top
    (make-rect -1 -1 -1 -1 false 0 0 false empty))
   "Rectangle after perfect bounce from left top should be returned")
  (check-equal?
   (bounce-rect-from-top (make-rect 401 -1 1 -1 false 0 0 false empty))
   (rect-after-perfect-bounce-from-right-top
    (make-rect 401 -1 1 -1 false 0 0 false empty))
   "Rectangle after perfect bounce from right top should be returned")
  (check-equal?
   (bounce-rect-from-bottom (make-rect -1 301 -1 1 false 0 0 false empty))
   (rect-after-perfect-bounce-from-left-bottom
    (make-rect -1 301 -1 1 false 0 0 false empty))
   "Rectangle after perfect bounce from left bottom")
  (check-equal?
   (bounce-rect-from-bottom (make-rect 401 301 1 1 false 0 0 false empty))
   (rect-after-perfect-bounce-from-right-bottom
    (make-rect 401 301 1 1 false 0 0 false empty))
...
(define (rect-after-perfect-bounce-from-left-top rect)
  (move-rectangle
   rect
   rect-after-pen-down-perfect-bounce-left-top
   rect-after-pen-up-perfect-bounce-left-top))
#|
(define (rect-after-perfect-bounce-from-left-top rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-perfect-bounce-left-top rect)
      (rect-after-pen-up-perfect-bounce-left-top rect)))|#

(define (rect-after-perfect-bounce-from-left-bottom rect)
  (move-rectangle
   rect
   rect-after-pen-down-perfect-bounce-left-bottom
   rect-after-pen-up-perfect-bounce-left-bottom))
#|
(define (rect-after-perfect-bounce-from-left-bottom rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-perfect-bounce-left-bottom rect)
      (rect-after-pen-up-perfect-bounce-left-bottom rect)))|#

(define (rect-after-perfect-bounce-from-right-top rect)
  (move-rectangle
   rect
   rect-after-pen-down-perfect-bounce-right-top 
   rect-after-pen-up-perfect-bounce-right-top))
#|
(define (rect-after-perfect-bounce-from-right-top rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-perfect-bounce-right-top rect)
      (rect-after-pen-up-perfect-bounce-right-top rect)))|#

(define (rect-after-perfect-bounce-from-right-bottom rect)
  (move-rectangle
   rect
   rect-after-pen-down-perfect-bounce-right-bottom 
   rect-after-pen-up-perfect-bounce-right-bottom))
#|
(define (rect-after-perfect-bounce-from-right-bottom rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-perfect-bounce-right-bottom rect)
      (rect-after-pen-up-perfect-bounce-right-bottom rect)))|#
#|
;; rect-after-perfect-bounce-from-direction :
;; Rectangle (Rectangle -> Rectangle) (Rectangle -> Rectangle) -> Rectangle
;; GIVEN: the state of a rectangle and two functions
;; RETURNS: the state of the rectangle after a perfect bounce
;;          from the corner of the canvas with reversed velocitites.
;; STRATEGY : Use template for Rect on rect
(define (rect-after-perfect-bounce-from-direction rect func1 func2)
  (if (rect-pen-down? rect)
      (func1 rect)
      (func2 rect)))|#

;; rect-after-pen-down-perfect-bounce-left-top : Rectangle -> Rectangle
;; rect-after-pen-down-perfect-bounce-left-bottom : Rectangle -> Rectangle
;; rect-after-pen-down-perfect-bounce-right-top : Rectangle -> Rectangle
;; rect-after-pen-down-perfect-bounce-right-bottom : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of the rectangle after pen down
;;          and a perfect bounce from the corner of
;;          the canvas having a added dot in list of 
;;          dots and reversed velocitites.
;; STRATEGY : Use template for Rect on rect
(define (rect-after-pen-down-perfect-bounce-left-top rect)
...
(define (rect-after-pen-down-perfect-bounce-left-bottom rect)
  (make-rect LEFT-TANGENT
             BOTTOM-TANGENT
             (* REVERSE (rect-vx rect))
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-down-perfect-bounce-right-top rect)
...
(define (rect-after-pen-down-perfect-bounce-right-bottom rect)
  (make-rect RIGHT-TANGENT
             BOTTOM-TANGENT
             (* REVERSE (rect-vx rect))
             (* REVERSE (rect-vy rect))
...
;; rect-after-pen-up-perfect-bounce-left-top : Rectangle -> Rectangle
;; rect-after-pen-up-perfect-bounce-left-bottom : Rectangle -> Rectangle
;; rect-after-pen-up-perfect-bounce-right-top : Rectangle -> Rectangle
;; rect-after-pen-up-perfect-bounce-right-bottom : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of the rectangle after pen up
;;          and a perfect bounce from the corner of
;;          the canvas having same list of dots and
;;          reversed velocitites.
;; STRATEGY : Use template for Rect on rect
(define (rect-after-pen-up-perfect-bounce-left-top rect)
  (make-rect LEFT-TANGENT
             TOP-TANGENT
             (* REVERSE (rect-vx rect))
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-up-perfect-bounce-left-bottom rect)
  (make-rect LEFT-TANGENT
             BOTTOM-TANGENT
             (* REVERSE (rect-vx rect))
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-up-perfect-bounce-right-top rect)
  (make-rect RIGHT-TANGENT
             TOP-TANGENT
             (* REVERSE (rect-vx rect))
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-up-perfect-bounce-right-bottom rect)
  (make-rect RIGHT-TANGENT
             BOTTOM-TANGENT
             (* REVERSE (rect-vx rect))
             (* REVERSE (rect-vy rect))
...
(define (rect-after-normal-bounce-top rect)
  (move-rectangle
   rect
   rect-after-pen-down-normal-bounce-top
   rect-after-pen-up-normal-bounce-top))
#|
(define (rect-after-normal-bounce-top rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-normal-bounce-top rect)
      (rect-after-pen-up-normal-bounce-top rect)))|#

(define (rect-after-normal-bounce-bottom rect)
  (move-rectangle
   rect
   rect-after-pen-down-normal-bounce-bottom 
   rect-after-pen-up-normal-bounce-bottom))
#|
(define (rect-after-normal-bounce-bottom rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-normal-bounce-bottom rect)
      (rect-after-pen-up-normal-bounce-bottom rect)))|#

(define (rect-after-normal-bounce-left rect)
  (move-rectangle
   rect
   rect-after-pen-down-normal-bounce-left 
   rect-after-pen-up-normal-bounce-left))
#|
(define (rect-after-normal-bounce-left rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-normal-bounce-left rect)
      (rect-after-pen-up-normal-bounce-left rect)))|#

(define (rect-after-normal-bounce-right rect)
  (move-rectangle
   rect
   rect-after-pen-down-normal-bounce-right 
   rect-after-pen-up-normal-bounce-right))
#|
(define (rect-after-normal-bounce-right rect)
  (if (rect-pen-down? rect)
      (rect-after-pen-down-normal-bounce-right rect)
      (rect-after-pen-up-normal-bounce-right rect)))|#
#|
;; rect-after-normal-bounce-direction :
;; Rectangle (Rectangle -> Rectangle) (Rectangle -> Rectangle) -> Rectangle
;; GIVEN: the state of a rectangle and two functions
;; RETURNS: the state of the rectangle after a smooth bounce
;;          from the edges of the canvas.
;; STRATEGY : Use template for Rect on rect
(define (rect-after-normal-bounce-direction rect func1 func2)
  (if (rect-pen-down? rect)
      (func1 rect)
      (func2 rect)))|#

;; rect-after-pen-down-normal-bounce-top : Rectangle -> Rectangle
;; rect-after-pen-down-normal-bounce-bottom : Rectangle -> Rectangle
;; rect-after-pen-down-normal-bounce-left : Rectangle -> Rectangle
;; rect-after-pen-down-normal-bounce-right : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of the rectangle after pen down
;;          and a normal bounce from the corner of
;;          the canvas having a added dot in the list
;;          of dots and changed velocitites.
;; STRATEGY : Use template for Rect on rect
(define (rect-after-pen-down-normal-bounce-top rect)
  (make-rect (+ (rect-x rect) (rect-vx rect))
             TOP-TANGENT
             (rect-vx rect)
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-down-normal-bounce-bottom rect)
  (make-rect (+ (rect-x rect) (rect-vx rect))
             BOTTOM-TANGENT
             (rect-vx rect)
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-down-normal-bounce-left rect)
  (make-rect LEFT-TANGENT
             (+ (rect-y rect) (rect-vy rect))
             (* REVERSE (rect-vx rect))
             (rect-vy rect)
...
(define (rect-after-pen-down-normal-bounce-right rect)
  (make-rect RIGHT-TANGENT
             (+ (rect-y rect) (rect-vy rect))
             (* REVERSE (rect-vx rect))
             (rect-vy rect)
...
;; rect-after-pen-up-normal-bounce-top : Rectangle -> Rectangle
;; rect-after-pen-up-normal-bounce-bottom : Rectangle -> Rectangle
;; rect-after-pen-up-normal-bounce-left : Rectangle -> Rectangle
;; rect-after-pen-up-normal-bounce-right : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of the rectangle after pen up
;;          and a normal bounce from the corner of
;;          the canvas having same list of dots and
;;          changed velocitites.
;; STRATEGY : Use template for Rect on rect                                      
(define (rect-after-pen-up-normal-bounce-top rect)
  (make-rect (+ (rect-x rect) (rect-vx rect))
             TOP-TANGENT
             (rect-vx rect)
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-up-normal-bounce-bottom rect)
  (make-rect (+ (rect-x rect) (rect-vx rect))
             BOTTOM-TANGENT
             (rect-vx rect)
             (* REVERSE (rect-vy rect))
...
(define (rect-after-pen-up-normal-bounce-left rect)
  (make-rect LEFT-TANGENT
             (+ (rect-y rect) (rect-vy rect))
             (* REVERSE (rect-vx rect))
             (rect-vy rect)
...
(define (rect-after-pen-up-normal-bounce-right rect)
  (make-rect RIGHT-TANGENT
             (+ (rect-y rect) (rect-vy rect))
             (* REVERSE (rect-vx rect))
             (rect-vy rect)
...
;; perfect and normal bounce on pen up
(define rect-bounce-left
  (make-rect 32 150 -4 4 false 0 0 false empty))
(define rect-bounce-right
  (make-rect 369 150 4 4 false 0 0 false empty))
(define rect-bounce-top
  (make-rect 32 26 -4 -4 false 0 0 false empty))
(define rect-bounce-bottom
  (make-rect 32 269 -4 4 false 0 0 false empty))
(define rect-bounce-top-1
  (make-rect 135 28 -4 -4 false 0 0 false empty))
(define rect-bounce-bottom-1
  (make-rect 202 273 4 4 false 0 0 false empty))
(define rect-bounce-right-bottom
  (make-rect 367 273 4 4 false 0 0 false empty))
(define rect-bounce-right-top
  (make-rect 367 10 20 -14 false 0 0 false empty))
(define rect-bounce-left-bottom
  (make-rect 10 273 4 4 false 0 0 false empty))

(define bounce-list
  (list rect-bounce-left rect-bounce-right
...
(define bounce-world
  (make-world bounce-list false))

(define bounce-world-after-tick
  (make-world
   (rectangle-after-tick bounce-list) false))

;; perfect and normal bounce on pen down
(define list-left
  (list (make-dot 32 150)))
(define list-right
  (list (make-dot 369 150)))
(define list-top
  (list (make-dot 135 28)))
(define list-bottom
  (list (make-dot 202 273)))
(define list-left-top
  (list (make-dot 32 26)))
(define list-right-bottom
  (list (make-dot 367 273)))
(define list-right-top
  (list (make-dot 367 10)))
(define list-left-bottom
  (list (make-dot 10 273)))

(define rect-pen-down-bounce-left
  (make-rect 32 150 -4 4 false 0 0 true list-left))
(define rect-pen-down-bounce-right
  (make-rect 369 150 4 4 false 0 0 true list-right))
(define rect-pen-down-bounce-top
  (make-rect 135 28 -4 -4 false 0 0 true list-top))
(define rect-pen-down-bounce-bottom
  (make-rect 202 273 4 4 false 0 0 true list-bottom))
(define rect-pen-down-perfect-bounce-left-top
  (make-rect 32 26 -4 -4 false 0 0 true list-left-top))
(define rect-pen-down-perfect-bounce-right-bottom
  (make-rect 367 273 4 4 false 0 0 true list-right-bottom))
(define rect-pen-down-perfect-bounce-right-top
  (make-rect 367 10 20 -14 false 0 0 true list-right-top))
(define rect-pen-down-perfect-bounce-left-bottom
  (make-rect 10 273 4 4 false 0 0 true list-left-bottom))

(define pen-down-bounce-list
  (list rect-pen-down-bounce-left
        rect-pen-down-bounce-right
        rect-pen-down-bounce-top
        rect-pen-down-bounce-bottom
        rect-pen-down-perfect-bounce-left-top
        rect-pen-down-perfect-bounce-right-bottom
        rect-pen-down-perfect-bounce-right-top
        rect-pen-down-perfect-bounce-left-bottom))

(define pen-down-bounce-world
  (make-world pen-down-bounce-list false))

(define pen-down-bounce-world-after-tick
  (make-world
   (rectangle-after-tick pen-down-bounce-list) false))
...
(begin-for-test
  ...
  (check-equal?
   (world-after-tick pen-down-bounce-world)
   pen-down-bounce-world-after-tick
   "The world after pen down and smooth bounces"))
...
;;           a u KeyEvent
;; STRATEGY: combine simpler functions
(define (is-u-key-event? ke)
  (key=? ke "u"))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; world-after-key-event : WorldState KeyEvent -> WorldState
;; GIVEN: a world state w and a KeyEvent ke
;; RETURNS: the WorldState that should follow the given 
;;          worldstate after the given KeyEvent ke
...
(define (world-after-key-event w ke)
  (cond
    ...
    [(is-d-key-event? ke) (world-after-arrow-key-event w ke)]
    [(is-u-key-event? ke) (world-after-arrow-key-event w ke)]
    [else w]))
...
(begin-for-test
  ...
  (check-equal?
   ...
   "The world after right key event should increase the
    velocity by 2 ticks per tick")
  (check-equal?
   (world-after-key-event sample-world "d")
   (make-world (list pen-down-rect-1 rect-2) true)
   "The world after d key event should set the pen-down?
    field of selected rectangle true")
  (check-equal?
   (world-after-key-event
    (make-world (list pen-down-rect-1 rect-2) true)
    "u")                      
   sample-world
   "The world after u key event should set the pen-down?
    field of selected rectangle false"))
...
;; STRATEGY: Use template for World on w
(define (new-world w)
  (make-world (add-new-rectangle (world-rects w)) (world-paused? w)))
...
(define (rect-after-key-event rect ke)
  (cond
    ...
    [(is-d-key-event? ke) (rect-after-d-key-event rect)]
    [(is-u-key-event? ke) (rect-after-u-key-event rect)]))
...
;; rect-after-d-key-event : Rectangle -> Rectangle
;; rect-after-u-key-event : Rectangle -> Rectangle
;; GIVEN: the state of a rectangle
;; RETURNS: the state of given rectangle followed 
;;          after various KeyEvent.
;; STRATEGY: Use template for Rect on rect
...
(define (rect-after-d-key-event rect)
  (make-rect (rect-x rect)
             (rect-y rect)
             (rect-vx rect)
             (rect-vy rect)
...
(define (rect-after-u-key-event rect)
  (make-rect (rect-x rect)
             (rect-y rect)
             (rect-vx rect)
             (rect-vy rect)
...
(define world-after-drag
  (make-world (list
               (rectangle-after-drag rect-1-after-mouse-event 40 50))
              false))
...
(define selected-unpaused-world-after-button-up
  (make-world
   (list rect-1-after-button-up rect-2 unselected-rect-button-up)
   false))
...
(begin-for-test
  ...
  (check-equal?
   ...
   "The rectangles should have unchanged selected coordinate fields after
    button-down outside the rectangles")
  (check-equal?
